...
`telrobot:init` 负责初始化本地 Telrobot CLI 运行环境。它会识别当前 OS 和 CPU 架构，从服务器下载对应的可执行文件，安装到 `~/.telrobot/bin`，并写入 `~/.telrobot/config.json`。

`telrobot:task` 是任务管理入口。它应该读取 `~/.telrobot/config.json`，通过其中的 `executablePath` 调用 Telrobot CLI，而不是假设系统 `PATH` 里一定存在 `telrobot-cli` 命令。

这样做的好处是安装过程不会修改 shell profile、用户环境变量或系统 PATH。用户如果要在终端手动执行，可以直接使用完整路径，例如 `~/.telrobot/bin/telrobot-cli`；Agent 内部则统一从配置文件读取执行路径。
...
```text
~/.telrobot/bin/telrobot-cli
~/.telrobot/config.json
```
...
可以用 `TELROBOT_HOME` 覆盖 Telrobot 本地目录，也可以用 `TELROBOT_CONFIG_PATH` 覆盖配置文件路径。

## Token 配置

`telrobot:init` 包含本地 token 配置功能。它会检查本地 Telrobot CLI 可执行文件是否存在，然后把 token 和执行路径写入 `~/.telrobot/config.json`。

在 `skills/telrobot-init` 目录执行：
...
```text
https://releases.telrobot.com/latest/<asset>
```
